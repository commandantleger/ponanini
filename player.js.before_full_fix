...
function startPlayerArrival() {
    ...
    player.x = 25 * Game.tileSize;
    player.y = 30 * Game.tileSize;
    player.moving = false;
...
function updateArrivalParticles(dt) {
    ...
    playerArrival.particles = playerArrival.particles.filter(p => p.life > 0);
}

function updatePlayer() {

    if (typeof gameFinished !== "undefined" && gameFinished)
        return;
    ...
    if (
        ty >= 0 && ty < WORLD.length &&
        tx >= 0 && tx < WORLD[0].length
    ) {
        const tile = WORLD[ty][tx];

        if (tile === "D" && currentMap === "village" && bridgeOpen) {
            onPortal = true;

            if (!portalMessage) {
                portalMessage = true;
                openDialogue("🌲 <b>Passage vers la forêt</b><br><br>Appuie sur E pour entrer.");
            }
...
function drawArrivalPortal(x, y) {
    ...
    glow.addColorStop(0, "rgba(100,130,255,.35)");
    ...
    ctx.strokeStyle = "rgba(160,180,255,.95)";
    ctx.lineWidth = 7;
    ctx.beginPath();
    ctx.ellipse(0, 0, 52, 78, 0, 0, Math.PI * 2);
    ctx.stroke();

    ctx.strokeStyle = "rgba(225,230,255,.55)";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.ellipse(0, 0, 40, 64, 0, 0, Math.PI * 2);
    ctx.stroke();
...
function drawArrivalImpact(x, y) {
    ...
    const progress = 1 - playerArrival.impact;

    ctx.save();
    ctx.strokeStyle = `rgba(230,235,255,${playerArrival.impact})`;
    ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.ellipse(x, y + 38, 25 + progress * 80, 8 + progress * 20, 0, 0, Math.PI * 2);
    ctx.stroke();
    ...
    playerArrival.particles.forEach(p => {
        ...
        ctx.fillRect(
            ...
            y + 38 - Math.sin(p.angle) * p.distance,
            p.size,
            p.size
        );
    });
...
function drawDuckBody(ctx, cx, cy) {
    ...
    ctx.beginPath();
    ctx.ellipse(cx, cy + 10, 14, 13, 0, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.ellipse(cx - 10, cy + 9, 6, 9, -.2, 0, Math.PI * 2);
    ctx.fill();

    ctx.fillStyle = "#d88732";
    ctx.fillRect(cx - 10, cy + 20, 7, 3);
...
function drawPlayer() {
    ...
    const groundY = player.y - Game.camera.y;
    const y = groundY - (playerArrival.active ? playerArrival.z : 0);
    const cx = x + 20;
    ...
    ctx.beginPath();
    ctx.ellipse(cx, groundY + 39, 17, 5, 0, 0, Math.PI * 2);
    ctx.fill();

    ctx.save();

    if (player.direction === "left") {
        ctx.translate(cx, y + 20 + bob);
        ctx.scale(-1, 1);
        drawDuckBody(ctx, 0, 0);
    } else if (player.direction === "up") {
        ctx.fillStyle = "#d8b94f";
        ctx.beginPath();
        ctx.ellipse(cx, y + 30 + bob, 14, 13, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#c5a845";
        ctx.beginPath();
        ctx.arc(cx, y + 13 + bob, 12, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#b89b3e";
        ctx.fillRect(cx - 14, y + 24 + bob, 7, 14);
        ctx.fillRect(cx + 7, y + 24 + bob, 7, 14);
    } else {
        drawDuckBody(ctx, cx, y + 20 + bob);
    }
