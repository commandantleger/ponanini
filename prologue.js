...
,

        {
            ...
            text:
                "Mais l'histoire ne commence pas à Ponan. " +
                "Elle commence dans un autre monde. Un humain ordinaire " +
                "franchit un portail temporel instable et fut projeté " +
                "dans une dimension qu'il ne connaissait pas.",
    ...
    drawStranger() {
        ...
        const cx = width / 2;
        const portalY = height * .30;

        const glow = ctx.createRadialGradient(
            cx, portalY, 10,
            cx, portalY, height * .42
        );

        glow.addColorStop(0, "rgba(100,130,255,.30)");
        ...
        ctx.strokeStyle = "rgba(170,190,255,.95)";
        ctx.lineWidth = 7;
        ctx.beginPath();
        ctx.ellipse(
            cx, portalY,
        ...
        ctx.beginPath();
        ctx.arc(cx, height * .61, 28, 0, Math.PI * 2);
        ctx.fill();

        ctx.strokeStyle = "rgba(190,210,255,.55)";
        ctx.lineWidth = 2;
        for (let i = 0; i < 10; i++) {
            const angle = this.visualTime * 1.5 + i * .62;
            const radius = 80 + (i % 3) * 35;
            const x = cx + Math.cos(angle) * radius;
            const y = portalY + Math.sin(angle) * radius * .55;
            ctx.beginPath();
            ctx.moveTo(x, y);
            ctx.lineTo(x + 8, y - 4);
            ctx.stroke();
